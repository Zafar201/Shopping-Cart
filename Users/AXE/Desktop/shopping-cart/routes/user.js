...
router.get('/',async function(req, res, next) {
  ...
  console.log(user)
  let cartCount=null
  if(req.session.user){//if there is user
...
router.post('/login',(req,res)=>{
  userHelpers.doLogin(req.body).then((response)=>{
    if (response.status){
      ...
      req.session.loginErr="Invailed username or password"
      res.redirect('/login')
    }
  })
})
...
router.post('/place-order',async(req,res)=>{
  let products=await  userHelpers.getCartProducList(req.body.userId)
